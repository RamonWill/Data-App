...
class RegistrationForm(FlaskForm):
    ...
    password = PasswordField("New Password", [v.DataRequired(), v.Length(min=5), v.EqualTo("confirm", message="Passwords must match")])
...
class WatchlistItemsForm(FlaskForm):
    ...
    ticker = StringField("Ticker", validators=[v.Length(min=2, max=20), v.InputRequired()])
    quantity = IntegerField("Quantity", validators=[v.InputRequired(), v.NumberRange(min=-10000000, max=10000000)])
    price = DecimalField("Price", validators=[v.InputRequired(), v.NumberRange(min=0, max=100000)])
    trade_date = DateTimeField("Trade Date", default=default_date)
    sector = SelectField("Sector",  validators=[v.InputRequired()])
    comments = TextAreaField("Comments", validators=[v.Optional(), v.Length(max=140)])
    ...
    def validate_trade_date(self, trade_date):
        ...
        if trade_date > max_date:
            raise v.ValidationError("The trade and time cannot be a date in the future")
        elif trade_date < lowest_date:
            raise v.ValidationError("The trade and time must be within 100 days of the order creation date")
...
class WatchlistGroupForm(FlaskForm):
    name = StringField("Watchlist Name", validators=[v.InputRequired(), v.Length(min=1, max=25)])
    ...
    def validate_name(self, name):
        ...
        if name_check is not None:
            raise v.ValidationError("You can not create two watchlists with the same name!")
